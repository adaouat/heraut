...
// TestGenerator_GenerateChangelog_IncrementalRefreshesTitle covers ADR-0050 (supersedes ADR-0038's
// "preamble preserved verbatim" line): an ordinary incremental splice now re-renders the preamble
// from current config on every run, same as a full regenerate — a hand-edited title/subtitle in the
// on-disk file does not survive the next `heraut changelog` run.
func TestGenerator_GenerateChangelog_IncrementalRefreshesTitle(t *testing.T) {
	mr := exectest.NewMockRunner()
	...
	g := New(mr, &config.ContentDriver{
		Output:             outPath,
		EffectiveTemplates: map[string]string{"title": "# Configured Title"},
	}, ModeChangelog)

	body, err := g.Generate("v1.1.0", nil)
	require.NoError(t, err)

	assert.True(t, strings.HasPrefix(body, "# Configured Title\n\n"),
		"an ordinary incremental splice re-renders the preamble from current config every run")
	assert.NotContains(t, body, "Totally Custom Title")
	assert.NotContains(t, body, "Hand-edited subtitle")
}
...
func TestGenerate_ReleaseFooterOverride(t *testing.T) {
	mr := exectest.NewMockRunner()
	mr.QueueResponse("v1.0.0\n", "", nil)               // previousTag
	mr.QueueResponse("2026-01-01T00:00:00Z\n", "", nil) // tagDate
	mr.QueueResponse(record("abc1234567", "A", "a@example.com", "2026-01-02T00:00:00Z", "feat: add x", ""), "", nil)
...
func TestGenerate_DefaultDocumentFooterCreditsHeraut(t *testing.T) {
	...
	mr.QueueResponse(record("abc1234567", "A", "a@example.com", "2026-01-02T00:00:00Z", "feat: add x", ""), "", nil)
	mr.QueueResponse("bob@x\n", "", nil) // authorsBefore
	g := New(mr, &config.ContentDriver{HerautVersion: "9.9.9"}, ModeReleaseNotes)
	g.now = func() time.Time { return time.Date(2026, 7, 6, 14, 30, 0, 0, time.UTC) }

	out, err := g.Generate("v1.1.0", nil)
	require.NoError(t, err)
	assert.Contains(t, out,
		"_Generated by [heraut](https://github.com/adaouat/heraut) 9.9.9 at 14:30 on 2026-07-06._",
		"the built-in document-level footer credits heraut with version + timestamp, with no override configured")
}

func TestGenerate_DocumentFooterContextIsHerautOnly(t *testing.T) {
	mr := exectest.NewMockRunner()
	mr.QueueResponse("v1.0.0\n", "", nil)
	mr.QueueResponse("2026-01-01T00:00:00Z\n", "", nil)
	mr.QueueResponse(record("abc1234567", "A", "a@example.com", "2026-01-02T00:00:00Z", "feat: add x", ""), "", nil)
	mr.QueueResponse("bob@x\n", "", nil)
	g := New(mr, &config.ContentDriver{
		EffectiveTemplates: map[string]string{"footer": "{{ .Tag }}"}, // .Tag exists on Release, not tplHeraut
	}, ModeReleaseNotes)
	_, err := g.Generate("v1.1.0", nil)
	require.Error(t, err, "the document-level footer only receives .Heraut's own fields — .Tag must fail to execute")
	assert.Contains(t, err.Error(), "footer")
}

// TestGenerateChangelog_DocumentFooterOnceAcrossMultipleSections proves the exact bug caught after
// shipping the default footer: a bootstrap/regenerate that renders multiple release sections must
// credit heraut exactly once, at the true end of the document — not once per rendered section (the
// original mistake: footer lived on the per-section root, so every historical section carried its
// own copy).
func TestGenerateChangelog_DocumentFooterOnceAcrossMultipleSections(t *testing.T) {
	dir := t.TempDir()
	out := filepath.Join(dir, "CHANGELOG.md")
	mr := exectest.NewMockRunner()
	mr.QueueResponse("v1.0.0\n", "", nil)                                                                      // scopedTags
	mr.QueueResponse(record("aaa1111111", "A", "a@x", "2026-02-01T00:00:00Z", "feat: brand new", ""), "", nil) // new: v1.0.0..HEAD
	mr.QueueResponse(record("bbb2222222", "B", "b@x", "2026-01-01T00:00:00Z", "fix: an old bug", ""), "", nil) // historical: ""..v1.0.0
	g := New(mr, &config.ContentDriver{Output: out, RegenerateChangelog: true, HerautVersion: "9.9.9"}, ModeChangelog)
	g.now = func() time.Time { return time.Date(2026, 7, 6, 14, 30, 0, 0, time.UTC) }

	body, err := g.Generate("v1.1.0", nil)
	require.NoError(t, err)
	assert.Equal(t, 1, strings.Count(body, "_Generated by [heraut]"),
		"the document footer must appear exactly once across a multi-release changelog, not once per section")
	assert.True(t, strings.HasSuffix(strings.TrimRight(body, "\n"),
		"_Generated by [heraut](https://github.com/adaouat/heraut) 9.9.9 at 14:30 on 2026-07-06._"),
		"the document footer sits at the true end of the file, after every section")
}

// TestGenerator_GenerateChangelog_IncrementalRefreshesDocumentFooter covers ADR-0050: an ordinary
// incremental splice now re-renders the document footer from current config on every run, replacing
// (not duplicating) whatever stale footer was on disk from an earlier bootstrap/regenerate.
func TestGenerator_GenerateChangelog_IncrementalRefreshesDocumentFooter(t *testing.T) {
	mr := exectest.NewMockRunner()
	mr.QueueResponse("v1.0.0\n", "", nil) // scopedTags for the splice path's latest lookup
	mr.QueueResponse(record("ccc3333333", "C", "c@example.com",
		"2026-03-01T00:00:00Z", "feat: another one", ""), "", nil) // new range v1.0.0..HEAD
	...
	outPath := filepath.Join(dir, "CHANGELOG.md")
	existing := "# Changelog\n\n<!-- heraut-release: v1.0.0 -->\n## [1.0.0]\n\n### Features\n\n- first\n" +
		footerAnchor + "\n_Generated by [heraut](https://github.com/adaouat/heraut) 0.59.0 at 12:00 on 2026-01-01._\n"
	require.NoError(t, os.WriteFile(outPath, []byte(existing), 0o644))

	g := New(mr, &config.ContentDriver{Output: outPath, HerautVersion: "9.9.9"}, ModeChangelog)
	g.now = func() time.Time { return time.Date(2026, 7, 6, 14, 30, 0, 0, time.UTC) }

	body, err := g.Generate("v1.1.0", nil)
	require.NoError(t, err)
	assert.Equal(t, 1, strings.Count(body, "_Generated by [heraut]"),
		"the refreshed footer replaces the stale one, it does not duplicate it")
	assert.NotContains(t, body, "0.59.0", "the stale on-disk footer is discarded, not preserved")
	assert.True(t, strings.HasSuffix(strings.TrimRight(body, "\n"),
		"_Generated by [heraut](https://github.com/adaouat/heraut) 9.9.9 at 14:30 on 2026-07-06._"),
		"the footer is re-rendered fresh from current config and the current run's clock")
}
