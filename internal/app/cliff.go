...
// content driver and mode ("changelog" or "release-notes"). cfg and env are used to
// derive the build-id postprocessor (when versioning.tag_format contains {build}) so
// the output matches what the pipeline actually feeds git-cliff.
//
...
func EffectiveCliffConfig(cfg *config.Config, driver *config.ContentDriver, mode, env string) (string, error) {
