# Archived task detail — roadmap.md phases

> Moved from [`docs/tasks/roadmap.md`](../roadmap.md) on 2026-08-29 to keep the live
> file lean. Covers every fully-closed Phase except the five that already point to a
> dedicated roadmap (23, 24, 25, 27, 29 — those stay in `roadmap.md` itself) and Phase
> 10's still-open CHECKPOINT K item (v1.0.0 cut), which also stays live there. See the
> live file's "Progress at a glance" table for the current status index.
