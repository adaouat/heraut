# Archived task detail — forge-abstraction-roadmap.md

> Moved from [`docs/tasks/forge-abstraction-roadmap.md`](../forge-abstraction-roadmap.md) on 2026-08-29 to keep the live file lean.
> T164 (the one still-open item) stays in the live file as a pointer; everything below is
> `Done`. See the live file's "Progress at a glance" table for the current status index.
