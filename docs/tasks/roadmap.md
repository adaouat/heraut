...
against T321's dead end.

#### ✦ `[x]` T323: bundle completions/man pages into the Homebrew cask via a dedicated tar.gz archive

**Completion note (2026-09-26).** Landed, but not the shape first proposed. The initial plan (see
git history of this entry) was to switch `homebrew_casks:` to `brews:` (Formula), following vhs's
config verbatim. That plan was wrong on inspection: `goreleaser check` on the real installed
`goreleaser` (2.16.0) reported `brews` as a hard-removed deprecation — GoReleaser's own docs say to
use `homebrew_casks:` instead, specifically because Cask picked up Linuxbrew support that Formula
used to be the only way to get. vhs's config predates that deprecation. So the "Cask is macOS-only,
Formula also gets us Linux" reasoning in the original proposal was backwards — corrected here rather
than shipped as a mistake.

The actual fix is smaller than proposed: `homebrew_casks:` already has static `completions:`
(bash/zsh/fish) and `manpages:` fields — the same fields T321 tried and hit "symlink source ...
is not there" on, purely because there was no archive for the referenced paths to live in. Added a
second `archives:` entry (`id: homebrew`, `formats: tar.gz`, `files: [LICENSE.md, completions/*,
manpages/*]`) alongside the existing raw-binary one, fed by four new `before.hooks` entries running
`go run ./cmd/heraut completion {bash,zsh,fish}` and `go run ./cmd/heraut man | gzip`.
`homebrew_casks[].ids: [homebrew]` points the cask at that archive; `completions:`/`manpages:`
reference the bundled files directly — confirmed against the installed `goreleaser` JSON schema
(`goreleaser jsonschema`) rather than guessed, since the docs site returned 404s for the exact page.
The raw-binary `archives:` entry (`id: default`) is untouched, so the direct curl/mise install path
doesn't change; ADR-0013 gained a note on both the archive addition and on why its self-update
rationale no longer applies (`internal/selfupdate` was already removed, per ADR-0014).

Verified with a real local install, not just a snapshot build: `goreleaser release --snapshot
--clean --skip=publish,validate,announce,sign,sbom` produced the tar.gz with the right file layout,
then `brew tap-new` plus a hand-built cask pointing at a `file://` URL for the local archive was
installed via `brew install --cask` against a scratch tap (renaming the binary/zsh-completion
targets to avoid colliding with this machine's real, already-installed `heraut` cask). Binary,
manpage, and all three completions linked in well under a second — no hang, confirming the static
fields never execute the installed binary, unlike `generate_completions_from_executable`. Separately
confirmed *why* T321's dynamic-completions attempt hung: the installed binary still carries
`com.apple.quarantine` (`xattr -l`), and actually running it (`heraut-local-test --version`) hung
until the flag was removed. That's the pre-existing, unrelated Gatekeeper/notarization gap already
called out in the README's "Prebuilt binary" caveat — this task doesn't touch it, only the install
step, which never runs the binary. Test cask, scratch tap, and `dist/` output were all cleaned up
afterward; the real production `heraut` v0.69.1 install was untouched throughout. `completions/` and
