...
fully-resolved display line, with any matching `rendering.commit.trailers` rule already applied
(a matching `renderer` executed, a matching `hide` entry already dropped from the list); print
`.Line` rather than composing `.Token`/`.Value` yourself, so your template stays correct however
a project has configured `rendering.commit.trailers` — see [Spec 02 §
