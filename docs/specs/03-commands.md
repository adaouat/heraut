...
| Flag                     | Description                                                                          |
|--------------------------|--------------------------------------------------------------------------------------|
| `--set-version`          | Override the auto-computed version for **any** strategy. Bypasses bump resolution entirely — no git calls are made to resolve it. Accepts any non-empty value with no whitespace; the value is rendered through the active strategy's tag shape exactly like an auto-resolved version would be: through the effective `tag_format` when one applies (per-env strategies, or a top-level `tag_format`; an optional leading `v` is stripped first, since a template has no single prefix), otherwise through `versioning.tag_prefix` (default `"v"` for SemVer strategies, `""` for CalVer; the configured prefix is what is stripped and re-applied, so `--set-version v1.2.3` with `tag_prefix: "rel-"` gives `rel-v1.2.3`). A full tag already carrying the right prefix round-trips unchanged. |
