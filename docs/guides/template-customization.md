...
| `footer` | root `Release` | Trailing content after the body | `_Generated by [heraut](url) version at HH:MM on YYYY-MM-DD._` |
...
wall-clock — the built-in `footer` renders it (`at HH:MM on YYYY-MM-DD`), so the footer's
timestamp changes on every regeneration even with no new commits, by design: it credits
heraut with *when* the file was last generated, not just *by what version*. Null the block
(`footer: ""`) if you'd rather keep changelog diffs limited to real content changes.
...
**Deterministic footer (version only, no timestamp), release notes only:** the built-in
default footer renders `.Heraut.GeneratedAt`, which changes every run — override it to drop
the clause if you'd rather the footer only change when heraut's own version does:
