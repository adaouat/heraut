...
changelog contains. Note the context: `{{ .Version }}` here (unlike in `release_header` above)
would resolve to heraut's own running version, not any particular release's — `title`/`subtitle`
execute against `.Heraut` directly, not a `Release`.
